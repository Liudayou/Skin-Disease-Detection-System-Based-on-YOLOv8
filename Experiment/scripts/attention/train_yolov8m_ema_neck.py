#!/usr/bin/env python3
"""
注意力消融实验 — EMA 注意力模块插入 Neck (PAN-FPN)。
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--model-cfg",
        type=str,
        default=str(PROJECT_ROOT / "GraduationScripts" / "models" / "yolov8m_ema_neck.yaml"),
        help="EMA-Neck 模型 YAML",
    )
    ...
    p.add_argument("--no-amp", action="store_true", help="禁用 AMP（可绕过 AMP checks 卡住的问题）")
    p.add_argument("--device", type=str, default="", help="留空则交给 Ultralytics 自动选择")
    p.add_argument("--seed", type=int, default=0)
    p.add_argument(
        "--run-name",
        type=str,
        default="yolov8m_ema_neck_ham_univ30_lowMixup015",
        help="实验名前缀（写入独立子目录与 CSV）；默认会自动加上 _e{epochs}",
    )
