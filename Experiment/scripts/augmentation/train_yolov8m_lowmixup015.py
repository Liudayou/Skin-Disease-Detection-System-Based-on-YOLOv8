#!/usr/bin/env python3
"""
数据增强消融实验 — lowMixup015 配方。

mosaic=1.0、mixup=0.15、close_mosaic=10、scale=0.5。
默认 patience=0 关闭早停，跑满 epochs。
输出到 results/model_scaling/yolov8m/train_results/lowMixup015/。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--run-name",
        type=str,
        default="yolov8m_lowmixup015_ham_univ30",
    ...
    p.add_argument(
        ...
        type=float,
        default=0.5,
        help="几何 scale；设为负数则不向 run_training 传入 scale（使用包内默认）",
    )
...
def _eval_on_test(*, run_name: str, data_yaml: Path, imgsz: int, batch: int, device: str | None) -> Path:
    ...
    out_dir = PROJECT_ROOT / "results" / "model_scaling" / "yolov8m" / "test_results" / "lowMixup015" / Path(run_name).name
...
def main() -> None:
    ...
    full_run_name = f"model_scaling/yolov8m/train_results/lowMixup015/{tag}"
