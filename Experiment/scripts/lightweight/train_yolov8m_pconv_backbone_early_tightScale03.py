#!/usr/bin/env python3
"""
轻量化消融实验 — PConv-early Backbone 替换实验。
...
for _ in range(3):
    ...
    raise RuntimeError(
        "Cannot locate GraduationScripts root (missing yolov8m_train_utils.py)."
    )
...
from modules.pconv_fasternet import PConv
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--model-cfg",
        type=str,
        default=str(PROJECT_ROOT / "GraduationScripts" / "models" / "yolov8m_pconv_backbone_early.yaml"),
    )
    ...
    p.add_argument(
        "--run-name",
        type=str,
        default="yolov8m_pconv_backbone_early_ham_univ30_tightScale03",
    )
