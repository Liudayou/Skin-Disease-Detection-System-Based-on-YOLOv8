#!/usr/bin/env python3
"""训练 YOLOv8m：Ultralytics 默认数据增强（本项目唯一数据增强基线）。
...
**早停**：默认 `--patience 0` 关闭早停，训练会跑满 `--epochs`；其它 `scripts/` 下脚本仍使用 `run_training` 默认 `patience=50`。

输出目录
- ``model_scaling/yolov8m/train_results/<run_name>_e{epochs}``
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--run-name", type=str, default="yolov8m_default_aug_ham_univ30")
...
def main() -> None:
    ...
    full_run_name = f"model_scaling/yolov8m/train_results/{tag}"
