#!/usr/bin/env python3
"""训练 YOLOv8s：Ultralytics 默认数据增强（本项目唯一数据增强基线）。
...
输出目录
- ``model_scaling/yolov8s/train_results/<run_name>_e{epochs}``
"""
...
def _default_weights() -> str:
    local = PROJECT_ROOT / "scripts" / "models" / "yolov8s.pt"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--run-name", type=str, default="yolov8s_default_aug_ham_univ30")
...
def main() -> None:
    ...
    full_run_name = f"model_scaling/yolov8s/train_results/{tag}"
