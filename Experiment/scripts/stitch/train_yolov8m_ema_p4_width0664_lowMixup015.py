#!/usr/bin/env python3
"""组合方案四：EMA P4 + width0664 + lowMixup015。

方案说明：
- 基于 YOLOv8m 基线，仅在 Neck P4 阶段插入 EMA 注意力模块
- 通道缩放 width_multiple=0.664 实现约 17% 参数量压缩
- 数据增强采用 lowMixup015（mosaic=1.0, mixup=0.15, scale=0.5）
- 追求精度与效率兼顾的部署方案

模型：
- `scripts/models/generated/attention_ablations/yolov8m_ema_p4_width0664.yaml`

训练增强（默认 lowMixup015）：
- mosaic=1.0, mixup=0.15, close_mosaic=10, scale=0.5

输出：
- results/combination/train_results/scheme4_ema_p4_width0664_lowMixup015/
- runs/detect/combination/scheme4_ema_p4_width0664_lowMixup015_e{epochs}/weights/{best,last}.pt

可选：
- `--eval-test` / `--eval-test-only` 写入 results/combination/test_results/scheme4_ema_p4_width0664_lowMixup015/test_metrics.csv
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--model-cfg",
        type=str,
        default=str(PROJECT_ROOT / "scripts" / "models" / "generated" / "attention_ablations" / "yolov8m_ema_p4_width0664.yaml"),
    )
    ...
    p.add_argument(
        "--run-name",
        type=str,
        default="scheme4_ema_p4_width0664_lowMixup015",
...
def _eval_on_test(*, run_name: str, data_yaml: Path, imgsz: int, batch: int, device: str | None) -> Path:
    ...
    out_dir = PROJECT_ROOT / "results" / "combination" / "test_results" / "scheme4_ema_p4_width0664_lowMixup015"
...
def main() -> None:
    ...
    full_run_name = f"combination/train_results/scheme4_ema_p4_width0664_lowMixup015/{tag}"
