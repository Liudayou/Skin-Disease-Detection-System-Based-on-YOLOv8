...
def main():
    ...
    p.add_argument("--model-cfg", default=str(PROJECT_ROOT/"scripts"/"models"/"generated"/"attention_ablations"/"{YAML_NAME}"))
    ...
    p.add_argument("--run-name", default="attention_ablation__{ATTN}_{STAGE}_e100")
...
for f in glob.glob('Experiment/scripts/attention/ablations/train_yolov8m_*.py'):
    bn = os.path.basename(f)
    attn = bn.split('_')[2]
    stage = bn.split('_')[3].split('.')[0]
    with open(f, 'w') as out:
        out.write(template.replace('{YAML_NAME}', f"yolov8m_{attn}_{stage}.yaml").replace('{ATTN}', attn.upper()).replace('{STAGE}', stage.upper()))
